...
# FUNCTIONS
def get_group_size(subsession: Subsession):
    return subsession.session.config.get('num_bidders', Constants.num_bidders)


def set_groups_for_all_rounds(subsession: Subsession):
    size = get_group_size(subsession)
    ...
    #shuffle(all_g)
    subsession.session.vars['all_pay_all_g'] = all_g
...
def set_groups_for_curr_round(subsession: Subsession):
    ...
    all_g = subsession.session.vars['all_pay_all_g']
...
def creating_session(subsession: Subsession):
    if subsession.round_number == 1:
        ...
        if all(k in subsession.session.config for k in ('bid_step', 'num_bidders')):
            bid_step = subsession.session.config['bid_step']
            num_bidders = subsession.session.config['num_bidders']
            # Sanity check
            if num_bidders == 5 and bid_step != 1:
                raise ValueError(f'If number of bidders is {num_bidders}, bid step should be 1.')
        elif any(k in subsession.session.config for k in ('bid_step', 'num_bidders')):
            raise KeyError(
...
def set_payoffs(group: Group):
    players = [p for p in group.get_players() if not p.participant.vars.get('is_dropout')]
...
def set_bid(player: Player):
    values = player.participant.vars['values_all_pay'][player.round_number-1]
...
class Bid(TimedPage):
    ...
    def vars_for_template(player: Player):
        num_bidders = player.session.config.get('num_bidders', Constants.num_bidders)
